...
class UserProfileFormRequest extends FormRequest
{
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            if (isset($data['id']) && $data['id'] == 1 && isset($data['is_enabled']) && !$data['is_enabled'])
                $validator->errors()->add('id', trans('php-core::validation.not_allowed'));

            if (isset($data['password']) && !empty($data['password']) && !preg_match('/^((?=.*[0-9])(?=.*[a-z|A-Z]))^.*$/', $data['password']))
                $validator->errors()->add('password', trans('php-core::validation.password_rule', ['attribute' => $attributes['password']]));
            if (isset($data['password_current']) && !empty($data['password_current'])) {
                if (!preg_match('/^((?=.*[0-9])(?=.*[a-z|A-Z]))^.*$/', $data['password_current'])) {
                    ...
                    if ($user && !Hash::check($data['password_current'], $user->password)) {
